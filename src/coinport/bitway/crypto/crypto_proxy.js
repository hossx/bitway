...
CryptoProxy.prototype.constructWithdrawal_ = function(transferReq, callback) {
    var self = this;
    Async.parallel ([
        function(cb) {self.getUnspent_.bind(self)(cb)},
...
CryptoProxy.prototype.getUnspent_ = function(callback) {
    var self = this;
    self.rpc.listUnspent(self.minConfirm, CryptoProxy.MAX_CONFIRM_NUM, [], function(unspentError, unspentReply) {
        if (unspentError) {
...
CryptoProxy.prototype.getCCTXFromTx_ = function(tx, callback) {
    ...
    Async.map(tx.vin, self.getInputAddress_.bind(self), function(error, rawInputs) {
        if (error) {
            ...
            cctx.sigId = sigId;
            cctx.theoreticalMinerFee = Math.ceil(tx.hex.length/2000.0)*0.0001;
...
CryptoProxy.prototype.makeNormalResponse_ = function(type, currency, response) {
    switch (type) {
        ...
        case BitwayResponseType.SEND_RAW_TRANSACTION:
            return new BitwayMessage({currency: currency, sendRawTransactionResult: response});
        default:
...
CryptoProxy.prototype.sendRawTransaction = function(request, callback) {
    var self = this;
    self.log.info('** Send Raw Transaction Request Received **');
    self.log.info("sendRawTransaction req: " + JSON.stringify(request));

    console.log('>>>>> request : ', request);
    this.send_.bind(self)(request.signedTx, function(error, ret) {
      if (error) {
        console.log('err', error);
        callback(self.makeNormalResponse_(BitwayResponseType.SEND_RAW_TRANSACTION, self.currency, new SendRawTransactionResult({error: ErrorCode.INVALID_SIGNED_TX})));
      } else {
        console.log('ret', ret);
        callback(self.makeNormalResponse_(BitwayResponseType.SEND_RAW_TRANSACTION, self.currency, new SendRawTransactionResult({error: ErrorCode.OK})));
      }
    })
};
