...
#scprit start
################################################################################
cd /var/coinport/bitway
git fetch && git rebase origin master
./activator clean assembly

# get jar versionid ---------------------------------------------------
version=`grep "val bitwayVersion"  /var/coinport/bitway/project/Build.scala | cut -d '"' -f2`

COMMAND="java -server $Xms $Xmx $NewSize $MaxNewSize $PermSize $MaxPermSize $UseParNewGC $UseConcMarkSweepGc $CMSInitOccupancyFraction $GCLog $GCStopTime $GCTimeStamps $GCDetails $CMSFullGCsBeforeCompaction -cp /var/coinport/bitway/bitway-backend/target/scala-2.10/bitway-backend-assembly-$version.jar -Dconfig.resource=application-test.conf com.coinport.bitway.Main 3551 127.0.0.1:3551 all 127.0.0.1"
