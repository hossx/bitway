...
class BlockchainDataManagerSpec extends Specification {
  import CryptoCurrencyAddressType._
  import TransferType._
  import TransferStatus._

  private def getBtcInternalAmount(amount: Double): Option[Long] = {
    Some((BigDecimal(amount) * 100000000).toLong)
  }

  private def getBtcExternalAmount(amount: Long): Option[Double] = {
    Some((BigDecimal(amount) / 100000000).toDouble)
  }

  "BlockchainDataManager" should {
    "address accocate test" in {
      val bwm = new BlockchainDataManager(Btc, 10)
    ...
    "get tx type test" in {
      ...
      bwm.getTransferType(Set("u1", "d1"), Set("h1", "u1")) mustEqual Some(Unknown)
      bwm.getTransferType(Set("u1", "d1"), Set("h1")) mustEqual Some(Withdrawal)
      bwm.getTransferType(Set("h1"), Set("c1", "h1")) mustEqual Some(Withdrawal)
      bwm.getTransferType(Set("c1"), Set("h1", "c1")) mustEqual Some(Unknown)
      ...
      bwm.getTransferType(Set("u1"), Set("d1")) mustEqual Some(Withdrawal)
      bwm.getTransferType(Set("u1"), Set("u2")) mustEqual Some(Unknown)
    }
    ...
    "getAddressStatus/getNetworkStatus/adjustAddressAmount/getReserveAmounts test" in {
      ...
      bwm.getReserveAmounts mustEqual Map(Hot -> getBtcInternalAmount(-1.4).get, Cold -> 0, User -> getBtcInternalAmount(1.2).get, Unused -> 0)
      ...
      bwm.getAddressStatus(Cold) mustEqual Map.empty[String, BlockIndex]
      bwm.getReserveAmounts mustEqual Map(Hot -> getBtcInternalAmount(-1.6).get, Cold -> 0, User -> getBtcInternalAmount(26.6).get, Unused -> 0)
    ...
    "recycleAddresses test" in {
      val bwm = new BlockchainDataManager(Btc, 10)
      bwm.faucetAddress(Unused, Set(CryptoAddress("d1", Some("p1")), CryptoAddress("d2", Some("p2")), CryptoAddress("d3", Some("p3")), CryptoAddress("d4", Some("p4")), CryptoAddress("d5", Some("p5")), CryptoAddress("d6", Some("p6"))))
      bwm.addressAllocated(1L, "d1")
      bwm.addresses mustEqual Map(Hot -> Set(), Cold -> Set(), User -> Set("d1"), Unused -> Set("d4", "d5", "d2", "d6", "d3"))
      bwm.addressTTL mustEqual Map()
      bwm.recycleAddresses(100)
      bwm.addressTTL mustEqual Map()
      bwm.addresses mustEqual Map(Hot -> Set(), Cold -> Set(), User -> Set("d1"), Unused -> Set("d4", "d5", "d2", "d6", "d3"))

      bwm.addressAllocated(2L, "d2", Some(10))
      bwm.addresses mustEqual Map(Hot -> Set(), Cold -> Set(), User -> Set("d1", "d2"), Unused -> Set("d4", "d5", "d6", "d3"))
      bwm.addressTTL mustEqual Map("d2" -> 10)

      bwm.recycleAddresses(10)
      bwm.addresses mustEqual Map(Hot -> Set(), Cold -> Set(), User -> Set("d1", "d2"), Unused -> Set("d4", "d5", "d6", "d3"))
      bwm.addressTTL mustEqual Map("d2" -> 10)

      bwm.recycleAddresses(11)
      bwm.addresses mustEqual Map(Hot -> Set(), Cold -> Set(), User -> Set("d1"), Unused -> Set("d4", "d5", "d2", "d6", "d3"))
      bwm.addressTTL mustEqual Map()
    }
  }
